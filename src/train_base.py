...
@torch.no_grad()
def ddim_sample(unet, x, f_multi, alpha_bar, T, device, n_steps=20, seed=42):
    """
...
def main():
    ...
    kd_unet_ckpt = 'checkpoints/blockwise_kd_unet.pth'
    if os.path.exists(kd_unet_ckpt):
        unet.load_state_dict(torch.load(kd_unet_ckpt, map_location=device)['unet'])
        print(f"Loaded blockwise distilled UNet from {kd_unet_ckpt}")
    else:
        print(f"Warning: {kd_unet_ckpt} not found. Training from scratch.")
