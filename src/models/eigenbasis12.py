...
class MultiLayerEigenbasis12(nn.Module):
    """12 eigenbases for Prithvi."""
    def __init__(self, teacher_ch, student_ch):
        super().__init__()
        assert len(teacher_ch) == len(student_ch) == 12
        self.bases = nn.ModuleList([
            GlobalEigenbasis(teacher_ch[i], student_ch[i]) for i in range(12)
        ])
