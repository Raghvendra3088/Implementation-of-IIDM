...
STRIDE       = 128
MIN_FOREST   = 0.2
CARBON_MEAN  = 45.0           # Mg C/ha — GEDI L4B global mean for subtropical forest
CARBON_FRAC  = 0.47           # IPCC default

# ── Huize County bounding box (WGS84) ─────────────────────────────────────────
HUIZE_BBOX = (103.0, 25.8, 103.8, 26.6)   # (west, south, east, north)
...
def step1_sentinel2():
    ...
    stacked = SRC / "data/processed/sentinel2/sentinel2_stacked.tif"
    if stacked.exists():
...
def step2_dem():
    ...
    dem_norm = SRC / "data/processed/dem/dem_normalized.tif"
    if dem_norm.exists():
    ...
    if not srtm.exists():
        srtm = SRC / "alos_dem/AP_25667_FBD_F0520_RT1/AP_25667_FBD_F0520_RT1.dem.tif"
...
def step3_canopy():
    ...
    canopy_norm = SRC / "data/processed/canopy_height/canopy_normalized.tif"
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP 4 — GEDI L4B: EXTRACT MEAN BIOMASS → CARBON
# ══════════════════════════════════════════════════════════════════════════════

def step4_gedi():
    print("\n" + "="*55)
    print("  STEP 4 — GEDI Biomass → Carbon Stock")
    print("="*55)

    # MU = mean biomass density (Mg/ha)
    gedi_mu = SRC / "GEDI_L4B_Gridded_Biomass_V2_1_2299_2.1-20260610_020255/GEDI04_B_MW019MW223_02_002_02_R01000M_MU.tif"

    dst = PROCESSED / "gedi_carbon.tif"
    reproject_clip(gedi_mu, dst, target_res=TARGET_RES)
    ...
    # Convert biomass → carbon using IPCC fraction
    carbon           = np.full_like(biomass, -9999.0)
    valid            = biomass != nodata
    carbon[valid]    = biomass[valid] * CARBON_FRAC

    valid_vals = carbon[carbon != -9999.0]
    global CARBON_MEAN
    CARBON_MEAN = float(valid_vals.mean()) if valid_vals.size > 0 else 45.0
    print(f"  GEDI mean carbon stock: {CARBON_MEAN:.2f} Mg C/ha")

    meta.update(dtype="float32", nodata=-9999.0, compress="lzw")
    with rasterio.open(dst, "w", **meta) as out:
        out.write(carbon[np.newaxis])
    print(f"  [SAVED] gedi_carbon.tif")
    return dst
...
def step6_mask(canopy_aligned):
    ...
    existing = SRC / "data/processed/canopy_height/forest_mask.tif"
    if existing.exists():
...
def step7_patches(aligned, mask_arr):
    ...
    for y in range(0, H - PATCH_SIZE + 1, STRIDE):
        for x in range(0, W - PATCH_SIZE + 1, STRIDE):
            ...
            if (inp == 0).mean() > 0.6:
                continue
            patches.append((inp.copy(), tgt.copy()))
    ...
    for split, sidx in splits.items():
        inp_d = PATCH_DIR / split / "input"
        tgt_d = PATCH_DIR / split / "target"
...
def step8_copy_raw():
    ...
    copies = [
        ...
        (SRC/"GEDI_L4B_Gridded_Biomass_V2_1_2299_2.1-20260610_020255/GEDI04_B_MW019MW223_02_002_02_R01000M_MU.tif",
                                                          RAW/"inventory/GEDI_L4B_MU.tif"),
    ]
