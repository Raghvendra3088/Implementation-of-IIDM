...
def extract_patches_for_split(input_stack, target, mask_arr, transform, regions_gdf, patch_size, stride, min_forest):
    ...
    for y in range(0, H - patch_size + 1, stride):
        for x in range(0, W - patch_size + 1, stride):
            mask_patch = mask_arr[0, y:y+patch_size, x:x+patch_size]
...
def main():
    ...
    # Use valid carbon values (>0) as the forest mask
    mask_arr = (carbon_arr > 0).astype(np.float32)
    ...
    for split in splits:
        ...
        regions = gpd.read_file(SPLITS_DIR / f"{split}_regions.geojson").to_crs(epsg=32648)
